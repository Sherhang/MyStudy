...
for step=2:steps
    ...
    % 上一时刻的载机位置和目标位置
    MPer.p = M.p - [M.v *dT.*cos(M.a), M.v *dT.*sin(M.a)];
    TPer.p = T.p - [T.v *dT.*cos(T.a), T.v *dT.*sin(T.a)];
    % 上一时刻的载机目标视线角
    qper = atan2(TPer.p(2)-MPer.p(2),TPer.p(1)-MPer.p(1)); % (-pi,pi]
    if qper<0
        qper = qper + 2*pi;
    end
    % 当前视线角
    q = atan2(T.p(2)-M.p(2),T.p(1)-M.p(1)); % (-pi,pi]
    if q<0
        q = q+2*pi;
    end
    % 视线角变化
    ...
    %----移动
    % 载机移动
    M.p = M.p+[M.v *dT.*cos(M.a), M.v *dT.*sin(M.a)];
...
print(figure1,'-dpng','-r300','./pngPNG/fighterToTargetPNG.png')   % 保存到工作目录下
%-----优势函数图像变化
...
print(figure2,'-dpng','-r300','./pngPNG/advancePNG.png')   % 保存到工作目录下
