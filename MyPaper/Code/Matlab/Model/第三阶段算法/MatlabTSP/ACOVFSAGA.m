...
function [planSAGA, fBestSave,fMeanSave,XcrossSave,XmutateSave, time] = ACOVFSAGA(TSPobj,popsize,steps,Pcross,Pmutate,Top,Dop)
% 输入obj，模型
...
%       fBestSave ，每一步最好的解记录
%       fMeanSave , 每一步平均解记录
% test: [planSAGA, fBestSave,fMeanSave, time] = GA1(TSPobj,100,100,1,0.5,"PMX","EM")
...
% 计算适应度函数
for i=1:popsize
 fitPop(i) = getDis(TSPobj, s(i,:)); 
end
%% 主循环开始
for step=1:steps
   % 先保存一些重要的变量
    [fBestSave(step), maxIndex] =  min(fitPop);
    ...
    [sCross,fitPop,Xcross] = cross(TSPobj,sSelect, s, fitPop, Pcross,Xcross); % 交叉,同时更新适应度等
    XcrossSave(step,:) = Xcross;
    [sMutate,fitPop,Xmutate] = mutate(TSPobj, sCross, fitPop, Pmutate,Xmutate);% 变异
...
toc;  % 计时结束
time = toc; % 足够精确
[fBestSave(step+1), maxIndex] =  min(fitPop);
fMeanSave(step+1) = mean(fitPop);
planSAGA = s(maxIndex,:); % 最优解保护
end

%% 轮盘赌选择算子,适应值越小，越容易被选到，最后适应值小的个数多
...
function [sCross,newFitPop,newX] = cross(TSPobj,s1, s2, fitPop, Pcross,X) % s1是选择之后的种群
% 输入：s(popsize,numLimit)老种群，sSelect(popsize,numLimit)选择之后的种群，Pcross交叉概率，crossAlg交叉算子选择,
% X算法的选择概率数列,fitPop对应的个体适应度, TSPobj 模型
% 常数，交叉概率0.4-0.99之间
...
%         crossHandle = @crossPMX; % DEBUG
        ...
        oldF = fitPop(i); % 老的解
         ...
         fitPop(i) = getDis(TSPobj, sCross(i,:)); 
        if fitPop(i)<=oldF
...
%% 群体变异
function [sMutate,newFit,newX] = mutate(TSPobj, s, fitPop, Pmutate,X)
...
% 计算距离
function dis = getDis(obj, plan)
dis = obj.MatrixP(plan(1), plan(obj.N));% 首和尾
for i=1:obj.N-1
    dis = dis + obj.MatrixP(plan(i),plan(i+1));
end
end