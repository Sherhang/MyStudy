...
function [planSAGA, fBestSave,fMeanSave, time] = SAGA(TSPobj,popsize,steps,Pcross,Pmutate, crossAlg, mutateAlg,Top,Dop)
% 输入obj，模型
...
for step=1:steps
   % 先保存一些重要的变量
    ...
    oldFit = fitPop;
    for i=1:popsize
        fitPop(i) = getDis(TSPobj, sMutate(i,:));   % ---------------------
        % 退火
        if fitPop(i)>oldFit(i)  % 产生了更差的解
            % 该解的接受概率 
            exp((oldFit(i)-fitPop(i))/Top)
...
% 基本算子部分
%% 轮盘赌选择算子,适应值越小，越容易被选到，最后适应值大的个数多
