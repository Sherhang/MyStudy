...
def main():
    ...
    algosName = [r'$\gamma_\alpha$ = 0.9', r'$\gamma_\alpha$ = 0.99', r'$\gamma_\alpha$ = 0.999',
                 r'$\gamma_\alpha$ = 0.9999',
                 r'$\gamma_\alpha$ = 0.99999', r'$\gamma_\alpha$ = 0.9', r'$\gamma_\alpha$ = 0.9', r'$\gamma_\alpha$ = 0.9']
    for k in range(Repeat):
        for i in range(tcom):
            func = algos[i]
            time1 = time.time()
            route, d = func(problem, steps, learning_rate=0.8, dsigma=0.9997, dalpha=gamma[i])  # 第二参数是迭代次数，route是list
            time2 = time.time()
            dt = time2-time1
    ...
    plt.savefig("./out_files/iterGammaAlpha.png", dpi=600, bbox_inches='tight')
    plt.show()
    # 数据写入文件
