...
legend('PMX\_EM最优个体','PMX\_EM种群均值','OX1\_EM最优个体','OX1\_EM种群均值'...
    ,'OX2\_EM最优个体','OX2\_EM种群均值','CX\_EM最优个体','CX\_EM种群均值');
%-----------比较变异算子---------
% PMX-EM
fB = zeros(Repetition,steps+1);
fM = zeros(Repetition,steps+1);
...
for p=1:numCross
    for q=1:numMutate
        ...
        fB =  mean(fB);
        fBestSave(p,q)= max(fB); % 平均最优值
        minSteps(p,q) = 0; % 最早得到近似最优解的时间
        for j=1:length(fB)
            if abs(fB(j)- fBestSave(p,q))<0.000001
                minSteps(p,q) = j;
                break;
            end
        end
        %         fMeanSave_PMX_EM = mean(fM);
        time(p,q) = mean(t); % 平均时间
    end
end
