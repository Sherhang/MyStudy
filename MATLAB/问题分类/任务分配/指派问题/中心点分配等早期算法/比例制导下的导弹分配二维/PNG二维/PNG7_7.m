...
theta_t=20*pi/180*[1 2 3 4 5 6 7]';%单位：rad，初始弹道偏角

X_t=20*[1 2 3 4 5 6 7 ]'; %单位：m，目标位置坐标，X轴
Y_t=30*[7 6 5 4 3 2 1]'; %单位：m，目标位置坐标，Y轴
...
dR=-1;
  ...
  for k=1:N
    ...
    r=sqrt((X_t(k)-X_m(k)).^2+(Y_t(k)-Y_t(k)).^2);  %r为x-y平面内导弹和目标间的距离
    delta_x=X_t(k)-X_m(k);
    delta_y=Y_t(k)-Y_m(k);
    theta_q(k)=atan(delta_y./delta_x);  %计算期望会合点的视线角
