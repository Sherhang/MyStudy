...
theta_m=200*pi/180*[1 2 3 4 5 6 7]';%单位：rad，初始弹道倾角
...
dR=-1;
  ...
  for k=1:N
    ...
    dR=((X_m(k)-X_t(k))*(dX_m-dX_t)+(Y_m(k)-Y_t(k))*(dY_m-dY_t))/r;
...
theta_t=20*pi/180*[1 2 3 4 5 6 7]'
for i=121:420 %当弹目相对距离增大时，仿真停止\
  for k=1:N
    dX_m=V_m(k)*cos(theta_m(k));
    dY_m=V_m(k)*sin(theta_m(k));
    dX_t=V_t(k)*cos(theta_t(k));
  ...
  n=n+1;
  t=t+dt;
  theta_t=theta_t-0.1*pi/180*[1 2 3 4 5 6 7]';
end
